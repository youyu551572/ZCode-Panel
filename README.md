...
- **添加账号两条线**：
  - BigModel 网页登录（手机号 + 短信验证码）
  - Z.ai 邮箱注册（走 `chat.z.ai`，需要能访问国际版）
...
关于两条线各自的判据，有两个地方是踩过坑才定下来的：

BigModel 的「登录 / 注册」按钮**一进页面就是可点的**，所以不能拿"按钮可点"当提交信号，否则会在你还没填完时空提交。必须两个输入框都有有效值才点。

Z.ai 的验证状态**无法从页面可靠判断**：阿里云验证组件的 DOM 会随框架重渲染整体消失又重建——实测填完表单 12 秒后验证入口的节点数直接归零，而验证根本没通过。所以驱动不去猜，改为留一段宽限期让你做完验证再去点，被拒就退避重试。
...
login-driver.js   登录窗口自动化驱动（BigModel 手机号 / Z.ai 邮箱注册）
...
- Z.ai 邮箱注册那条线走的是国际版 `chat.z.ai`，需要代理。面板会在开始前实测窗口的实际出口地区；系统代理只在浏览器生效时，这扇窗口仍然是直连的
